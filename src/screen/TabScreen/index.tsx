...
const INITIAL_TAB_DATA: TabData = {
  ...
  chordLib: {
    "G": { frets: [3, 2, 0, 0, 0, 3], theory: "I major" },
    "Gmaj7": { frets: [3, null, 4, 4, 3, null], theory: "I maj7" },
    "C": { frets: [null, 3, 2, 0, 1, 0], theory: "IV major" },
    "Cmaj7": { frets: [null, 3, 5, 4, 5, null], theory: "IV maj7" },
  },
  measures: [
    { id: 1, chord: "Gmaj7", lyrics: "這是一段測試歌詞", notes: [{ string: 6, fret: 3, beat: 0 }, { string: 4, fret: 4, beat: 1 }] },
    { id: 2, chord: "Cmaj7", lyrics: "準備進入副歌", notes: [{ string: 5, fret: 3, beat: 0 }, { string: 4, fret: 5, beat: 1 }] }
  ]
};
              ...
              <div className="flex flex-col gap-3">
                ...
                <div className="flex items-center gap-2">
                  {[4, 8, 16].map(num => (
                    <button 
                      key={num}
                      onClick={() => saveToHistory({...tabData, metadata: {...tabData.metadata, subdivisions: num}})}
                      className={`px-4 py-2 rounded-xl font-bold text-sm transition-all ${tabData.metadata.subdivisions === num ? 'bg-zinc-900 text-white shadow-lg' : 'bg-zinc-100 text-zinc-400 hover:bg-zinc-200'}`}
                    >
                      {num}
                    </button>
                  ))}
